...
def load_data(dset, normalize_features=True):
    ...
    path = osp.join('/root/workspace/lqy/NPNC/DND-NET-main/', 'Data', dset)
    ...
    if dset in ['arxiv']:
        dataset = PygNodePropPredDataset(root="/root/workspace/lqy/NPNC/DND-NET-main/Data", name = 'ogbn-arxiv')
        data = dataset[0]
        data.split_idx = dataset.get_idx_split()
    
    if dset in ['citeseer']:
        data = torch.load(f"/root/workspace/lqy/NPNC/DND-NET-main/Data/citeseer.pt")
        ...
        return data, meta
    if dset in ['cora_ml']:
        # /root/workspace/lqy/NPNC/DND-NET-main/Data
        data = torch.load(f"/root/workspace/lqy/NPNC/DND-NET-main/Data/cora_ml.pt", pickle_module=pickle)
        data.y = torch.squeeze(data.y)
        data.num_classes = torch.unique(data.y).shape[0]
